...
export default function WorkoutsPage() {
  ...
  const [workoutTableData, setWorkoutTableData] = useState<{
    [workoutId: string]: { percentages: number[]; weights: string[] } | null
  }>({})
  ...
  const workoutsForSelectedDate =
    advancedFilters.categories.length > 0 || advancedFilters.startDate || advancedFilters.endDate
      ? workouts
      : workouts.filter(
          (workout) => workout && workout.date && isSameDay(new Date(workout.date), selectedDate)
        )
  ...
  const handleTableData = (
    workoutId: string,
    data: { percentages: number[]; weights: string[] } | null
  ) => {
          ...
          <div className="grid grid-cols-7 gap-1">
            {weekDays.map((day) => {
              const dayWorkouts = workouts.filter(
                (workout) => workout && workout.date && isSameDay(new Date(workout.date), day)
              )
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {(advancedFilters.startDate || advancedFilters.endDate) && (
              <Badge variant="outline" className="gap-1">
                ...
                {advancedFilters.startDate && advancedFilters.endDate
                  ? `${format(new Date(advancedFilters.startDate), 'dd/MM', { locale: es })} - ${format(
          ...
          <div className="flex items-center justify-between">
            ...
            {workoutsForSelectedDate.length > 0 && (
              <span className="text-xs text-muted-foreground">
                {workoutsForSelectedDate.length} entrenamientos
              </span>
            )}
          </div>
